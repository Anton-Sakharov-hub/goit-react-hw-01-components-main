...
function createTheadMarkup(object) {
  let keys = Object.keys(object);
  keys = keys.filter(key => key !== 'id');
  // console.log(keys)
  const markup = keys.map(key => <th key={key}>{key.toUpperCase()}</th>);

  return markup;
}

// function createTbodyMarkup(arr) {
//   const tbodyMarkup = arr.map()
//   const values = Object.values(object);
//   console.log(values);
// }

function TransactionHistory({ transactions }) {
  // getObjectEntries(transactions[0])
  // createTbodyMarkup(transactions[0]);
  return (
